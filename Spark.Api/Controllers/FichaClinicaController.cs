    ...
    public class FichaClinicaController : ControllerBase
    {
        ...
        private Guid GetUsuarioLogadoId()
        {
            ...
            if (string.IsNullOrWhiteSpace(id) || !Guid.TryParse(id, out var userId))
                throw new UnauthorizedAccessException("Token sem identificador do usu�rio.");
        ...
        public async Task<IActionResult> GetByPacienteId([FromRoute] Guid pacienteId)
        {
            ...
            if (ficha == null)
                return BadRequest("Sem vinculo ou sem ficha."); // sem v�nculo (ou sem ficha)

            return Ok(new { Data = ficha });
        }

        /// <summary>
        /// Mant�m o uso do Handler como era antes.
        /// A �nica diferen�a: agora a controller injeta o usuarioLogadoId no command
        /// (o handler/repository precisa usar isso para validar v�nculo).
        /// </summary>
        [Route("")]
        [HttpPost]
